...
ssh $ssh_options_deploymachine root@$master_ip "
  mkdir -p $remote_code_dir &&
  mkdir -p $remote_config_dir &&
  mkdir -p $remote_exp_dir/raw-results" || exit 1

# Upload code to the master
rsync --quiet -rptz -e "ssh $ssh_options_deploymachine" $local_code_files "root@$master_ip:$remote_code_dir" || exit 2

# Upload config to the master
rsync --quiet -rptz -e "ssh $ssh_options_deploymachine" $exp_data_dir/config/* "root@$master_ip:$remote_config_dir" || exit 3

# Upload analysis scripts and queries to the master
rsync --quiet -rptz -e "ssh $ssh_options_deploymachine" queries scripts "root@$master_ip:$remote_work_dir" || exit 4

# Upload commands to the master
scp $ssh_options_deploymachine "$exp_data_dir/$local_master_command_file" "root@$master_ip:$remote_master_command_file" || exit 5
...
ssh $ssh_options_deploymachine root@$master_ip "
  ...
  echo "Setting path at PATH=\$PATH:$remote_gopath/bin:$remote_work_dir/bin:$remote_work_dir/go/bin" &&
  echo "Setting go at GOPATH=/opt/gopath" &&
  echo 'Compiling ISS.' &&
  export PATH=\$PATH:$remote_gopath/bin:$remote_work_dir/bin:$remote_work_dir/go/bin &&
